...
if __name__ == '__main__':
    # These modules should be defined elsewhere in your project.
    ...
    object_detection_module = GeneralObjectDetectionModule(source_module=image_reading_module,
    ...
    while True:
        if image_reading_module.value is not None:
            input_image = image_reading_module.value.copy()

            # Optional: mask out moving objects to avoid affecting optical flow.
            if object_detection_module.value:
                for obj in object_detection_module.value['moving_object_registry'].registry:
                    x1, y1, x2, y2 = obj.xyxy.astype(int)
                    input_image[y1:y2, x1:x2] = 0

            shifts = shift_processor.process(input_image.copy())
            # Display the shifts on the image.
            cv2.putText(input_image, f"H: {shifts[0]:.2f} px/s, V: {shifts[1]:.2f} px/s",
                        (10, input_image.shape[0] - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.imshow("Shift Measurement", input_image)
